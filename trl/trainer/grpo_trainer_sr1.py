...
def replace_context_with_answer_format(prompt: str, answer: str) -> str:
    # Extract content inside <format: name>...</format: name> from the answer
    match_format = re.findall(r"<format:\s*([^>]+)>(.*?)</format:\s*\1>", answer, re.DOTALL)
    if not match_format:
        return "say hi only" 

    format_content = "\n\n".join(content.strip() for _, content in match_format)
    ...
    new_prompt = f"""Use the context below to answer the question as clearly and accurately as possible.
                ...
                Respond with one or more reasoning steps inside <think>...</think> tags.  
                Then give the final answer inside an <answer>...</answer> tag.  
                Avoid unnecessary repetition or placeholder content.
...
class GRPOTrainerSR1(Trainer):
    ...
    def _generate_and_score_completions(
        ...
        for s_in, s_comp in zip(inputs_2, completions):
            s_in['prompt'] = replace_context_with_answer_format(s_in['prompt'], s_comp)

        prompts_2 = [x["prompt"] for x in inputs_2]
        original_prompts_2 = copy.deepcopy(prompts_2)
        prompts_text_2 = [maybe_apply_chat_template(example, self.processing_class)["prompt"] for example in inputs_2]
        ...
        if self.use_vllm:
            ...
            # Generate completions using vLLM: gather all prompts and use them in a single call in the main process
            if self.vllm_mode == "server":
                raise NotImplementedError("self.vllm_mode == 'server' not implemented")
            ...
            # Generate completions using colocated vLLM instances: each device holds vLLM copy and work on their own batch of prompts
                ...
                if self.vllm_tensor_parallel_size > 1:
                    # Gather prompts from all ranks in the TP group and flatten.
                    # Each rank starts with its own prompts; after gathering, all ranks see the full group set.
                    orig_size_2 = len(prompts_text_2)
                    gathered_prompts_2 = [None for _ in range(self.vllm_tensor_parallel_size)]
                    torch.distributed.all_gather_object(gathered_prompts_2, prompts_text_2, group=self.tp_group)
                    all_prompts_text_2 = [p for sublist in gathered_prompts_2 for p in sublist]
                else:
                    all_prompts_text_2 = prompts_text_2

                vllm_inputs_2 = all_prompts_text_2

                with profiling_context(self, "vLLM.generate"):
                    all_outputs_2 = self.llm.generate(vllm_inputs_2, sampling_params=sampling_params, use_tqdm=False)

                completion_ids_2 = [output.token_ids for outputs in all_outputs_2 for output in outputs.outputs]

                if self.vllm_tensor_parallel_size > 1:
                    # Slice completions for this rank within its TP group.
                    # Each rank generates all outputs — we keep only our share.
                    local_rank_in_group = torch.distributed.get_rank(group=self.tp_group)
                    tp_slice_2 = slice(local_rank_in_group * orig_size_2, (local_rank_in_group + 1) * orig_size_2)
                    completion_ids_2 = completion_ids_2[tp_slice_2]
            ...
            completion_ids_2 = pad(completion_ids_2, padding_value=self.pad_token_id)

        else:
            raise NotImplementedError("did not use vllm")

        # Mask everything after the first EOS token
        is_eos = completion_ids_2 == self.eos_token_id
        eos_idx = torch.full((is_eos.size(0),), is_eos.size(1), dtype=torch.long, device=device)
        eos_idx[is_eos.any(dim=1)] = is_eos.int().argmax(dim=1)[is_eos.any(dim=1)]
        sequence_indices = torch.arange(is_eos.size(1), device=device).expand(is_eos.size(0), -1)
        ...
        # Convert tensor to a list of lists of token IDs. This will be passed to the reward function, avoiding the need
        # to re-tokenize completions if the reward is computed from tokens.
        completion_ids_list_2 = [row[mask_row].tolist() for row, mask_row in zip(completion_ids_2, completion_mask_2.bool())]

        # Decode the generated completions
        completions_text_2 = self.processing_class.batch_decode(completion_ids_2, skip_special_tokens=True)
        if is_conversational(inputs_2[0]):
            completions_2 = []
            for prompt, completion in zip(prompts, completions_text_2):
                bootstrap = prompt.pop()["content"] if prompt[-1]["role"] == "assistant" else ""
                completions_2.append([{"role": "assistant", "content": bootstrap + completion}])
        else:
            completions_2 = completions_text_2

        rewards_per_func_2 = self._calculate_rewards(inputs_2, original_prompts_2, completions_2, completion_ids_list_2)
        
        rewards_per_func = rewards_per_func + self.sr1_bonus * rewards_per_func_2
